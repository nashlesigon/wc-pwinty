 ...
 *
 * @author v3 Steve Honey
 * @see https://github.com/SteveHoneyNZ
 * @version 3.0
 * @access public
 *
 * 
...
class WPPwinty {
  ...
  function WPPwinty() {
		...
		if ($api_env == "production") {
			$this->api_url = "https://sandbox.pwinty.com/v2"; // Changed to prevent live use of beta for now
		} else {
	...
	function createOrder( $recipientName, $address1, $address2, $addressTownOrCity, $stateOrCounty, $postalOrZipCode, $countryCode, $destinationCountryCode, $useTrackedShipping, $payment, $qualityLevel ) {
		...
		if (is_array($data["body"])) {
			if (isset($responseBody["errorMessage"])) {
				$this->last_error = $responseBody["errorMessage"];
				return 0;
			} else {
	...
	function getOrder( $id ) {
		...
		if (is_array($data['body'])) {
			if (isset($responseBody["error"])) {
				$this->last_error = $responseBody["error"];
				return 0;
			} else {
				return $data;
			}
