...
function register_pwinty_album_post_type() {
...
add_action( 'init', 'register_pwinty_album_post_type', 0 );

// Assign WooCommerce Categories and Tags for use with Pwinty Album Post Type
...
add_action( 'woocommerce_after_register_taxonomy', 'add_wc_category_tags_to_pwinty_album', 10 );
...
// Add custom order submit action button

function add_submit_to_order_admin_actions($actions) {
			
	  global $post;
	  global $the_order;  // globals maybe not ideal
